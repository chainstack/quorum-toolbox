...
def check_ipc_len(ipc):
    if len(ipc) > 64:
        raise Exception("ipc file ({0}) length ({1}) in config file too high. This may lead to bind errors when "
                        "tessera is launched.".format(ipc, len(ipc)))
...
tbin = os.path.join(os.environ['TESSERA_LOC'], 'tessera')
cmd = sh.Command('java').bake('-jar', tbin, '-configfile', tessera_config_file)

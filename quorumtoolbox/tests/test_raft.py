...
if raft_launch_params.get('raft') != '--raft':
    print(raft_launch_params)
    raise Exception('--raft flag error')

if raft_launch_params.get('rpcapi') != '--rpcapi raft':
    print(raft_launch_params)
    raise Exception('--rpcapi error')

if raft_launch_params.get('raftport') != make_node_param('--raftport', test_d['port']):
    print(raft_launch_params)
    raise Exception('--raftport error')

if raft_launch_params.get('raftblocktime') != make_node_param('--raftblocktime', test_d['block_time']):
    print(raft_launch_params)
...
if build_configuration['network']['peers'] != test_d['peers']:
    ...
    raise Exception('Peers expected {0} and actual {1} dont tally'.
                    format(test_d['peers'], build_configuration['network']['peers']))
...
if raft_launch_params.get('rpcapi') != make_node_param('--rpcapi', 'raft'):
    print(raft_launch_params)
    raise Exception('--rpcapi error')

if raft_launch_params.get('raftport') != make_node_param('--raftport', 50400):
    print(raft_launch_params)
    raise Exception('--raftport error')

if raft_launch_params.get('raftblocktime') != make_node_param('--raftblocktime', 50):
    print(raft_launch_params)
    raise Exception('--raftblocktime error')
